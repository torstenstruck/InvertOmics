...
### Usage of pipeline ###
# sh PurgeDups_QualityAssessment_v2.1.sh [Name of fastq file] [Name of primary assembly file] [Name of haplotig1 file] [Name of haplotig2 file] [Name to meryl database] [e value] [Maximum number of hits] [Maximum number of threads] [Genome size] >> PurgeDups_QualityAssessment_Pipeline.out
#
...
#
# example of usage: sh PurgeDups_QualityAssessment_v2.1.sh ../reads.fastq pri_assembly.fasta hap1_assembly.fasta hap2_assembly.fasta reads.fastq.meryl 1e-20 1 64 926M >> PurgeDups_QualityAssessment_Pipeline.out
#
...
echo -e "sh PurgeDups_QualityAssessment_v2.1.sh ${FASTQ} ${ASS} ${HAP1} ${HAP2} ${MERYL} ${EVALUE} ${MAXHITS} ${THREADS} ${GENOMESIZE}" >> PurgeDups_QualityAssessment_Pipeline.log
...
echo -e "\nRunning BUSCO analyses on the purged primary assembly using the following command line: busco -i ${ASM_PUR} -l metazoa_odb10 --out BUSCO_Metazoa_Primary_Purged -m geno -c ${THREADS} -f > Busco_${ASM_PUR}.log" >> PurgeDups_QualityAssessment_Pipeline.log
...
echo -e "\nblobtools create --fasta ${ASM_PUR} BlobtoolsResults_Primary_Purged" >> PurgeDups_QualityAssessment_Pipeline.log
echo -e "blobtools add --hits ${ASM_PUR}.blobplot.blast.out --taxrule bestsumorder --taxdump ./taxdump BlobtoolsResults_Primary_Purged" >> PurgeDups_QualityAssessment_Pipeline.log
echo -e "blobtools add --cov MappedReads_${ASM_PUR}.sorted.bam BlobtoolsResults_Primary_Purged" >> PurgeDups_QualityAssessment_Pipeline.log
echo -e "blobtools add --busco ./BUSCO_Metazoa_Primary_Purged/run_metazoa_odb10/full_table.tsv BlobtoolsResults_Primary_Purged" >> PurgeDups_QualityAssessment_Pipeline.log

echo -e "\n\n********** To display the data in a webserver do the following: **********" >> PurgeDups_QualityAssessment_Pipeline.log
echo -e "blobtools view --remote ./BlobtoolsResults_Primary_Purged" >> PurgeDups_QualityAssessment_Pipeline.log
