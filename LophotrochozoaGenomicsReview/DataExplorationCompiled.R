...
Species_Data_Input <- read_excel("./LophotrochozoaGenomesCompiled.xlsx")
...
ggsave(filename = paste0("Results/Counts_by_Phylum.pdf"), 
       plot = phylum_plot, 
       device = "pdf", 
...
for (Phylum in unique_phyla) {
  ...
  ggsave(filename = paste0("Results/Counts_by_Class_", gsub(" ", "_", Phylum), ".pdf"), 
         plot = p, 
         device = "pdf", 
         width = 29.7,  # Width for A4 in cm
         height = 21,   # Height for A4 in cm
...
write_xlsx(final_results, path = "Results/Class_Counts_By_Phylum.xlsx")
...
analyze_distribution <- function(data, column_name) {
  ...
  # Export to Excel
  write_xlsx(occurrence_counts, path = paste0("Results/", column_name, "_occurrences.xlsx"))
  ...
  # Save plot
  ggsave(filename = paste0("Results/", column_name, "_occurrences_plot.pdf"),
         plot = plot, 
         device = "pdf", 
         width = 29.7,  # Width for A4 in cm
         height = 21,   # Height for A4 in cm
...
for (i in 8:47) {
  ...
  ggsave(filename = paste0("Results/", column, "_All_Data_Plot.pdf"), plot = p_all_data, 
         device = "pdf", 
  ...
  write_xlsx(summary_stats_grouped, path = paste0("Results/", column, "_Summary_Statistics_By_Phylum.xlsx"))
  ...
  if (nrow(plot_data) > 0) {
    ...
    ggsave(filename = paste0("Results/", column, "_Grouped_by_Phylum_Plot.pdf"), plot = p_grouped_phylum, 
           device = "pdf", 
           width = 29.7,  # Width for A4 in cm
           height = 21,   # Height for A4 in cm
           units = "cm")  # Set units to centimeters
...
analyze_linear_regression <- function(raw_data, color, column_x, column_y) {
  ...
  # Checks for more than two orders of magnitude in both dimensions
...
#    scale_y_log10() +  # Log scale on y-axis
  ...
  dev.copy(pdf, file = paste0("Results/", column_y, "_vs_", column_x, "_Percentage_deviation.pdf"))
  dev.off()  
  ...
  ggsave(filename = paste0("Results/", column_y, "_vs_", column_x, "_Linear_regression.pdf"), plot = linreg_p, 
         device = "pdf", 
         width = 29.7,  # Width for A4 in cm
         height = 21,   # Height for A4 in cm
         units = "cm")  # Set units to centimeters
...
# Analyze chromosome_count vs. haploid_number_estimate
analyze_linear_regression(Species_Data_Input, "phylum", "haploid_number_estimate", "chromosome_count")
