...
### Usage of pipeline ###
# sh FilteredDatasets_QualityAssessment_v2.1.sh [Path and name to fastq file] [Name of assembly file] [Name to meryl database] [e value] [Maximum number of hits] [Maximum number of threads] [Genome size] > FilteredDatasets_QualityAssessment_Pipeline.out
#
...
#
# example of usage: sh FilteredDatasets_QualityAssessment_v2.1.sh ../reads.fastq assembly.fasta reads.fastq.meryl 1e-20 1 64 926M > FilteredDatasets_QualityAssessment_Pipeline.out
#
...
echo -e "sh FilteredDatasets_QualityAssessment_v2.0.sh ${FASTQ} ${ASS} ${MERYL} ${EVALUE} ${MAXHITS} ${THREADS} ${GENOMESIZE}" >> FilteredDatasets_QualityAssessment_Pipeline.log
...
blastn -task megablast -query ${ASSEMBLY} -db /storage/InvertOmics/00_Scripts/01_Databases/NCBI_nt/nt -outfmt '6 qseqid staxids bitscore std' -max_target_seqs ${MAXHITS} -max_hsps 1 -num_threads ${REDTHREADS} -evalue ${EVALUE} -out ${ASSEMBLY}.blobplot.blast.out &
local PID1=$!
...
echo -e "blastn -task megablast -query ${ASS} -db /storage/InvertOmics/00_Scripts/01_Databases/NCBI_nt/nt -outfmt '6 qseqid staxids bitscore std' -max_target_seqs ${MAXHITS} -max_hsps 1 -num_threads ${REDTHREADS} -evalue ${EVALUE} -out ${ASS}.blobplot.blast.out" >> FilteredDatasets_QualityAssessment_Pipeline.log
